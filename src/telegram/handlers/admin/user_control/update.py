...
async def edit_user_field_step1(message: Message, state: FSMContext):
	log.debug("Изменение свойства пользователя: {}. Запрос значения...".format(message.text))
...
async def edit_user_field_step2(message: Message, state: FSMContext):
	log.debug("Изменение свойства пользователя: {}. Проверка и установка значения".format(message.text))
